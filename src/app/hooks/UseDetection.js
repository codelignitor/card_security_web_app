...
//                 canProceedToBack:
//                   bufferedFrames >= 6 && chipDetected && bankLogoDetected,
//               });
...
//                 cleanup();
//                 console.log(
//                   `Front side complete - 6 frames buffered, chip detected, and bank logo detected`
//                 );
//                 resolve(apiResponse);
//                 return;
//               }
...
//                     setErrorMessage(
//                       "Card chip and bank logo not detected. Please ensure both the chip and bank logo are clearly visible and try again."
//                     );
//                   } else if (buffered >= 6 && !chip) {
//                     setErrorMessage(
//                       "Card chip not detected. Please ensure the chip is clearly visible and try again."
//                     );
//                   } else if (buffered >= 6 && !bankLogo) {
//                     setErrorMessage(
//                       "Bank logo not detected. Please ensure the bank logo is clearly visible and try again."
//                     );
//                   } else {
//                     setErrorMessage(
...
//             if (bufferedFrames >= 6 && !chipDetected && !bankLogoDetected) {
//               setErrorMessage(
//                 "Timeout: Chip and bank logo not detected. Please ensure both the chip and bank logo are clearly visible and try again."
//               );
//             } else if (bufferedFrames >= 6 && !chipDetected) {
//               setErrorMessage(
//                 "Timeout: Chip not detected. Please ensure the chip is clearly visible and try again."
//               );
//             } else if (bufferedFrames >= 6 && !bankLogoDetected) {
//               setErrorMessage(
//                 "Timeout: Bank logo not detected. Please ensure the bank logo is clearly visible and try again."
//               );
...
//                 if (apiResponse.complete_scan === true) {
//                   isComplete = true; // CRITICAL: Stop the loop
//                   cleanup(); // CRITICAL: Clear intervals and cleanup
//                   console.log(
//                     "🎯 Complete scan detected - Back side detection complete"
//                   );
//                   resolve(apiResponse);
//                   return;
//                 }
...
//                   if (phase === "back") {
//                     const buffered =
//                       lastApiResponse.buffer_info?.back_frames_buffered || 0;
...
//                           ", "
//                         )}). Please ensure the card's back side is clearly visible showing magnetic strip, signature strip, hologram, and customer service details.`
//                       );
//                       setCurrentPhase("error");
//                       reject(
//                         new Error(
...
//                     } else if (buffered < 6) {
//                       setErrorMessage(
//                         "Failed to capture sufficient frames for back side. Please try again."
//                       );
//                       setCurrentPhase("error");
//                       reject(
//                         new Error("Insufficient frames captured for back side")
//                       );
//                     } else {
//                       resolve(lastApiResponse);
//                     }
...
//               } else if (
//                 bufferedFrames >= 6 &&
//                 count < requiredBackSideFeatures
//               ) {
//                 setErrorMessage(
...
//                 setCurrentPhase("error");
//                 reject(
//                   new Error(`Timeout: Insufficient back side features detected`)
//                 );
//                 return;
//               }
//             }
        ...
        try {
          ...
          if (frame && frame.size > 0) {
            ...
            try {
              ...
              // Check for validation states in both message_state AND movement_state
          ...
          if (frame && frame.size > 0) {
            ...
            try {
              ...
              if (phase === 'back' && apiResponse.complete_scan === true) {
                // FIXED: Handle both "complete" and "retry" status
                if (apiResponse.ocr_validation === "complete") {
                  isComplete = true;
                  cleanup();
                  console.log(`Back side complete - complete_scan is true and status is complete`);
                  // Don't set phase here - let main component handle it
                  resolve(apiResponse);
                  return;
                } else if (apiResponse.ocr_validation === "retry" || apiResponse.ocr_validation === "retry_required") {
                  isComplete = true;
                  cleanup();
                  ...
                  setCurrentPhase('error'); // Use error phase, main component will handle retry logic
                  reject(new Error('Status is retry - need to restart from front scan'));
                  return;
                } else {
                  // Status is something else, check old conditions (3/4 features)
                  ...
                  if (count >= requiredBackSideFeatures) {
                    isComplete = true;
                    cleanup();
                    console.log(`Back side complete - complete_scan is true, status is ${apiResponse.ocr_validation}, and ${count}/4 features detected`);
                    // Don't set phase here - let main component handle it
                    resolve(apiResponse);
                    return;
                  } else {
                ...
                if (lastApiResponse) {
                  if (phase === 'back') {
                    if (lastApiResponse.complete_scan === true) {
                      if (lastApiResponse.ocr_validation === "complete") {
                        console.log('Max frames reached, complete_scan is true and status is complete');
                        setCurrentPhase('scan-complete');
                        resolve(lastApiResponse);
                      } else if (lastApiResponse.ocr_validation === "retry" || lastApiResponse.ocr_validation === "retry_required") {
                        console.log('Max frames reached, complete_scan is true but status is retry');
                        setErrorMessage('Scan needs to be retried. Please start the scanning process from front side again.');
                        setCurrentPhase('retry-required');
                        reject(new Error('Status is retry - need to restart from front scan'));
                      } else {
        ...
        if (!isComplete) {
          ...
          if (lastApiResponse) {
            ...
            if (phase === 'back') {
              if (lastApiResponse.complete_scan === true) {
                if (lastApiResponse.ocr_validation === "complete") {
                  console.log('Timeout reached, complete_scan is true and status is complete');
                  setCurrentPhase('scan-complete');
                  resolve(lastApiResponse);
                  return;
                } else if (lastApiResponse.ocr_validation === "retry" || lastApiResponse.ocr_validation === "retry_required") {
...
//               // NEW: Check for camera_work_done first
//               if (apiResponse.camera_work_done === true) {
//                 isComplete = true;
//                 cleanup();
//                 console.log('Camera work completed, showing loading screen...');
//                 setCurrentPhase('loading'); // Set to loading phase
//                 resolve(apiResponse);
//                 return;
//               }
